...
class ClientConfig(BaseModel):
    ...
    image_size: tuple = (128, 128)               # target image size should be used for training 
    ...
    lr: float = 0.001                            # 0.001 - face images | 0.00001 - medical images 
    ...
    lora_rank: int = 8                           # Lora/DyLora rank 4 - 16
    ...
    port: int = 8000                             # port number for server & clients to communicate 
    ...
    use_data_poisoning: int = 1                  # 0 - no data poisoning | 1 - use data poisoning
...
for i in range(len(labels)):
    ...
    elif 'ProxyFL-proxy' in labels[i]:
...
current_dir_acc_fig_file = os.path.join(f'./GraphsCL-{client_id}', acc_file_name)
...
for i in range(len(all_training_times)):
    ...
    if labels[i] == 'AvgPush':
        plt.plot(smoothed_x, smoothed_y_mean_time, label='AvgPush', color='green', linestyle='-', marker='o', markersize=9, markevery=max(1, len(smoothed_x) // 10))
    elif 'ProxyFL' in labels[i]:
        plt.plot(smoothed_x, smoothed_y_mean_time, label='ProxyFL', color='blue', linestyle='-', marker='o', markersize=9, markevery=max(1, len(smoothed_x) // 10))
...
current_dir_fig_file_train_time = os.path.join(f'./GraphsCL-{client_id}', train_time_file_name)
...
current_dir_fig_file_comm_time = os.path.join(f'./GraphsCL-{client_id}', comm_time_file_name)
# Create the directory if it doesn't exist
