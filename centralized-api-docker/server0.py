...
parser.add_argument("--clients_ips", type=str, required=True, help="Path to all_clients_ips JSON file")
...
try:
    with open(args.config, "r") as file:
        ...
        port = training_config.get("port", 8002)  # Default to 8002 if not found
        total_rounds = training_config.get("n_rounds", 100)  # Default to 100 if not found

        print(f"Loaded {args.config} successfully.")
except FileNotFoundError:
    print(f"Error: {args.config} not found.")
    exit(1)

try:
    with open(args.clients_ips, "r") as file:
        ...
        print(f"Loaded {args.clients_ips} successfully.")
except FileNotFoundError:
    print(f"Error: {args.clients_ips} not found.")
    exit(1)
...
################################### Euclidean Distance from the Global Model ##############################################33
...
def compute_global_model(all_model_weights):
    ...
    # ✅ Print first 10 values for readability
    #print("\n🌍 Global Model Weights (First 10 values):")
    #print(global_model[:10])
...
def broadcast_aggregated_weights(weights, round_number):
    ...
    print()        
    print(f"Aggregated weights for round {round_number} broadcasted to all clients.")
    print()
...
def start_receiving_messages(server_id, server_ip):
    ...
    checkpoints_dir = "./aggregated_weights"
    ...
    if os.path.exists(checkpoints_dir):
        ...
        if checkpoint_files:
            ...
            latest_weights_path = os.path.join(checkpoints_dir, f"aggregated_weights_round_{latest_round}.pth")
    ...
    if latest_weights_path:
        ...
        print(f"No checkpoint Round found. Starting from round 1.")
        print()
        start_round = 1
